...
void NvbloxMeshVisual::setCeilingCutoff(bool cut_ceiling, float ceiling_height)
{
...
NvbloxMeshVisual::getMeshColorFromColorAndNormal(
  ...
  const geometry_msgs::msg::Point32 & normal) const
{
  ...
  if (mesh_color_ == MeshColor::kColor) {
    ...
    const Ogre::Vector3 light_dir = Ogre::Vector3(0.8f, -0.2f, 0.7f).normalisedCopy();
    const Ogre::Vector3 light_dir2 = Ogre::Vector3(-0.5f, 0.2f, 0.2f).normalisedCopy();
    ...
    Ogre::Vector3 lambert = lambertShading(normal_pt, light_dir, color_pt) +
      lambertShading(normal_pt, light_dir2, color_pt) + ambient;
...
void NvbloxMeshVisual::setMessage(const nvblox_msgs::msg::Mesh::ConstSharedPtr & msg)
{
  ...
  for (size_t i = 0; i < msg->block_indices.size(); i++) {
    ...
    for (size_t i = 0; i < mesh_block.vertices.size(); ++i) {
      ...
      if (!mesh_block.normals.empty()) {
        ogre_object->normal(
          mesh_block.normals[i].x, mesh_block.normals[i].y,
...
void NvbloxMeshVisual::setFrameOrientation(const Ogre::Quaternion & orientation)
{
