...
def filter_out_upper_lower_percentile(array,
    ...
    keep_flags = np.logical_and((array < percentile_upper),
                                (array > percentile_lower))
    return array[keep_flags]


def make_results_table(topic_to_stamps_map,
                       cpu_percentages=None,
                       gpu_percentages=None,
    ...
    # Message frequencies
    def stamps_to_freq(stamps):
        return 1e9 / np.mean(np.diff(stamps))
    ...
    semantic_processed_freq = stamps_to_freq(topic_to_stamps_map[sem_name])
    ...
    table_dict = {
        'depth_released_freq': depth_released_freq,
