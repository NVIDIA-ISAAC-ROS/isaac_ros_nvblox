...
void EsdfSliceConverter::sliceLayersToCombinedDistanceImage(
    const EsdfLayer& layer_1, const EsdfLayer& layer_2, float layer_1_slice_height, 
    float layer_2_slice_height, Image<float>* output_image, AxisAlignedBoundingBox* aabb) {
  CHECK_NOTNULL(aabb);
  *aabb = esdf_slicer_.getCombinedAabbOfLayersAtHeight(layer_1, layer_2, 
    layer_1_slice_height, layer_2_slice_height);
