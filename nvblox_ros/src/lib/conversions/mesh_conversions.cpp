  ...
  nvblox_msgs::msg::Mesh * mesh_msg)
{
  ...
  meshMessageFromMeshBlocks(mesh_layer, block_indices, timestamp, frame_name, mesh_msg);
  // Setting the previous map to clear as we are resending the full map.
  mesh_msg->clear = true;
}
  ...
  nvblox_msgs::msg::Mesh * mesh_msg)
{
  ...
  for (size_t i_block = 0; i_block < num_blocks; ++i_block) {
    ...
    const int num_triangle_indices =
      serialized_mesh->getNumTriangleIndicesInBlock(i_block);
    ...
    for (size_t i_vert = 0; i_vert < num_vertices; ++i_vert) {
      mesh_msg->blocks[i_block].vertices[i_vert] = point32MessageFromVector(
        serialized_mesh->getVertex(
          i_block,
          i_vert));
    ...
    for (size_t i_tri = 0; i_tri < num_triangle_indices; ++i_tri) {
  ...
  for (size_t i_block = 0; i_block < num_blocks; ++i_block) {
      ...
      itr != serialized_mesh->triangleBlockItr(i_block + 1); ++itr)
    {
      marker.points.emplace_back(
        pointMessageFromVector(serialized_mesh->getVertex(i_block, *itr)));
      marker.colors.emplace_back(
        colorMessageFromColor(serialized_mesh->getColor(i_block, *itr)));
    }
